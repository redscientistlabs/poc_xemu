...
package_windows() {
    rm -rf qemudist
    mkdir -p qemudist
    cp i386-softmmu/qemu-system-i386.exe qemudist/qemu-system-i386.exe
    cp i386-softmmu/qemu-system-i386w.exe qemudist/qemu-system-i386w.exe
    cp x86_64-softmmu/qemu-system-x86_64.exe qemudist/qemu-system-x86_64.exe
    cp x86_64-softmmu/qemu-system-x86_64w.exe qemudist/qemu-system-x86_64w.exe
    cp -r "${project_source_dir}/data" qemudist/
    python3 "${project_source_dir}/get_deps.py" qemudist/qemu-system-i386.exe qemudist
    strip qemudist/qemu-system-i386.exe
    strip qemudist/qemu-system-i386w.exe
}
...
"${configure}" \
    --extra-cflags="-DXBOX=0 ${build_cflags} ${sys_cflags} ${CFLAGS}" \
